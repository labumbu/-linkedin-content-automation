...
- **Trends**: Web search runs via provider's search tool. Anthropic uses `client.beta.messages.create` with `betas: ["web-search-2025-03-05"]` and `max_tokens: 4096` (regular `client.messages.create` silently ignores the web_search tool). URLs extracted from `web_search_tool_result` content blocks; `block.content` is `Array | object` — always guard with `Array.isArray()`. Route has 55s fallback timeout. Reddit public API often returns 0 posts (403 blocked from server IPs). Trends cached 23h in DB; `?force=true` bypasses cache. `saveTrends()` deduplicates within-batch only.
